<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="设置项描述" prop="describe">
        <el-input
      ...
      <el-form-item label="设置内容" prop="values">
        <el-input
    ...
    <el-table v-loading="loading" :data="settingList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="设置项标示" align="center" prop="key" />
      <el-table-column label="设置项描述" align="center" prop="describe" />
      <el-table-column label="设置内容" align="center" prop="values" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="设置项描述" prop="describe">
          <el-input v-model="form.describe" placeholder="请输入设置项描述" />
        </el-form-item>
        <el-form-item label="设置内容" prop="values">
          <el-input v-model="form.values" placeholder="请输入设置内容" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        describe: [
          { required: true, message: "设置项描述不能为空", trigger: "blur" }
        ],
        values: [
          { required: true, message: "设置内容不能为空", trigger: "blur" }
        ],
        updateTime: [
  ...
  methods: {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const keys = row.key || this.ids;
      this.$confirm('是否确认删除【请填写功能名称】编号为"' + keys + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delSetting(keys);
        }).then(() => {
          this.getList();
